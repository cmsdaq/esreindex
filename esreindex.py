...
def scan2(client, query=None, scroll='5m', **kwargs):
    ...
    resp = client.search(body=query, search_type='scan', scroll=scroll, fields = "_routing,_source,_parent", **kwargs)
    ...
    while True:
        ...
        for hit in resp['hits']['hits']:
            if "fields" in hit.keys():
                hit["_routing"] = hit["fields"]["_routing"]
                hit["_parent"] = hit["fields"]["_parent"]
                del hit["fields"]
            yield hit
        scroll_id = resp['_scroll_id']
